  ...
  const handleAddPath = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        setPathMsg({ type: "error", text: data.message || "Failed to add path" });
      }
    } catch (e) {
    ...
    <div style={{ background: COLORS.bg, color: COLORS.text, minHeight: "100vh", padding: "24px 28px" }}>
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        {[
          ...
          { label: "NEW FILES",      value: totalNew,       accent: "#06b6d4" },
        ].map(card => (
      ...
      <div className="grid gap-3 mb-4" style={{ gridTemplateColumns: "1fr 2fr" }}>

        {/* Monitored Paths */}
        <Card>
          <CardLabel>MONITORED PATHS</CardLabel>
          <div style={{ display: "flex", flexDirection: "column", gap: "6px", marginBottom: "14px" }}>
            {paths.length === 0 ? (
              ...
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 10px", background: "#060d1a", borderRadius: "6px", border: `1px solid ${COLORS.border}` }}>
                ...
                <span style={{ fontFamily: "monospace", fontSize: "11px", color: COLORS.text, wordBreak: "break-all" }}>{p}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: "12px" }}>
            <div style={{ fontSize: "10px", color: COLORS.muted, letterSpacing: "0.07em", marginBottom: "8px" }}>ADD PATH</div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
              ...
              <tbody>
                {filteredAlerts.length === 0 ? (
                  <tr>
                    <td colSpan={4} style={{ padding: "20px 10px", color: COLORS.muted, textAlign: "center", fontSize: "12px" }}>
                      No alerts found
                    </td>
                  </tr>
                  ...
                  return (
                    <tr key={alert._id || i} style={{ borderBottom: `1px solid #080f1a` }}>
                      ...
                      <td style={{ padding: "8px 10px", fontFamily: "monospace", color: COLORS.text, fontSize: "11px", maxWidth: "300px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {alert.file}
                      </td>
                      <td style={{ padding: "8px 10px" }}>
